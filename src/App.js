...
import { FileUp, Trash2, Edit3, Printer, Layers } from 'lucide-react';
...
export default function WaterproofWarrantyApp() {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (newWarranties.length > 0) {
            setWarrantyList([...warrantyList, ...newWarranties]);
          }
        }
      });
    }
  ...
  const handleIndividualSave = (item) => {
    const originalTitle = document.title;
    document.title = `${item.customerName}新築防水保証書`;
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="mb-8 grid lg:grid-cols-3 gap-6 print:hidden">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 lg:col-span-2">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-orange-500"><Edit3 size={20}/> 発行リスト（個別PDF出力）</h2>
              ...
              {warrantyList.map((item, idx) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-slate-50 rounded border border-slate-200 text-xs font-bold">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleIndividualSave(item)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded flex items-center gap-2 shadow-sm transition-all">
                      <Printer size={14}/> PDF保存
                    </button>
        ...
        <div className="space-y-12 print:space-y-0">
          {warrantyList.map((item) => (
            <div 
              key={item.id} 
              className={`warranty-page bg-white shadow-2xl print:shadow-none w-[210mm] min-h-[297mm] p-[12mm] flex flex-col justify-between box-border print:break-after-page mx-auto relative
                ${printingId && printingId !== item.id ? 'hidden print:hidden' : 'flex'}
              `}
            >
              <div className="border-[4px] border-double border-slate-900 p-[6mm] flex flex-grow flex-col justify-between relative">
                ...
                <div>
                  ...
                  <div className="flex justify-between items-start mb-6">
                    ...
                    {/* ★タイトル：サイズを text-3xl に縮小し、trackingを調整 */}
                      ...
                      <div key={cell.field} className="border-b border-slate-400 pb-0.5">
                        ...
                        <div className="flex items-center">
                          <input 
                            type={cell.type || "text"} 
                            value={cell.value} 
                            onChange={(e) => updateItem(item.id, cell.field, e.target.value)}
                            className={`w-full text-[12px] bg-transparent outline-none font-bold ${cell.field === 'customerName' ? 'text-[16px]' : ''}`}
                          />
                ...
                <div className="mt-auto">
                  ...
                  <div className="flex justify-between items-end border-t-4 border-slate-900 pt-4 px-2">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] text-slate-800 mb-1 font-bold italic">
